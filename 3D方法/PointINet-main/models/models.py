...
class PointINet(nn.Module):
    ...
    def forward(self, points1, points2, features1, features2, t):
        ...
        points1_feature = points1[:, 3:, :].contiguous()  # 属性，只有一行
        points2_feature = points2[:, 3:, :].contiguous()
        points1 = points1[:, :3, :].contiguous()  # 坐标，有三行
        points2 = points2[:, :3, :].contiguous()

        # Estimate 3D scene flow
        with torch.no_grad():
            flow_forward = self.flow(points1, points2, features1, features2)  # 计算场景流用的点云的属性为0
